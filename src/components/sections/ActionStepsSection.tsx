        ...
        <div className="relative mx-auto max-w-6xl rounded-3xl overflow-hidden shadow-[0_40px_120px_rgba(1,40,64,0.45)]">
          ...
          <div className="relative text-white px-6 py-10 md:px-12 md:py-14 border border-white/10 rounded-3xl">
            ...
            <div className="text-center max-w-3xl mx-auto mb-10">
              ...
              <h2 className="text-3xl md:text-4xl font-philosopher font-bold leading-tight">
            ...
            <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto mb-10">
              {actions.map((action, index) => (
                <div key={index} className="group text-center rounded-2xl p-6 bg-white/10 border border-white/15 hover:border-yellow-400/40 transition-all duration-300 backdrop-blur-sm">
                  
                  <div className="relative w-14 h-14 mx-auto mb-4">
                    <div className="absolute inset-0 rounded-2xl bg-yellow-400/40 blur-xl opacity-80 group-hover:opacity-100 group-hover:blur-2xl" />
                    
                    <div className="relative w-14 h-14 rounded-2xl gradient-gold flex items-center justify-center shadow-[0_0_35px_rgba(234,179,8,0.65)]">
                      <action.icon className="h-7 w-7 text-secondary-foreground" />
                    </div>
                  </div>

                  <h3 className="font-bold text-lg mb-2">{action.title}</h3>
                  <p className="text-white/85 text-sm">{action.description}</p>
                </div>
              ))}
            </div>
